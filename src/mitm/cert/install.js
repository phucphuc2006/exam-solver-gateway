...
function checkCertInstalledWindows(certPath) {
  return new Promise((resolve) => {
    ...
    exec("certutil -store Root \"ES Gateway MITM Root CA\"", { windowsHide: true }, (error) => {
      resolve(!error);
    });
  });
}
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "ES Gateway MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function uninstallCertWindows() {
  ...
  return new Promise((resolve, reject) => {
    exec(
      `certutil -delstore Root "ES Gateway MITM Root CA"`,
      { windowsHide: true },
      (error) => {
...
function checkCertInstalledLinux() {
  const certFile = `${LINUX_CERT_DIR}/ES Gateway-root-ca.crt`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const destFile = `${LINUX_CERT_DIR}/ES Gateway-root-ca.crt`;
