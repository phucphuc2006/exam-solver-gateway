...
// Check if settings has ES Gateway config
const hasESGatewayConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["ES Gateway"];
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      settings,
      hasESGateway: hasESGatewayConfig(settings),
...
// POST - Update ES Gateway settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `ES Gateway/${model}`;
    ...
    // Update models.providers.es-gateway
    settings.models.providers["ES Gateway"] = {
...
// DELETE - Remove ES Gateway settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove ES Gateway from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["ES Gateway"];
    ...
    // Remove ES Gateway models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("ES Gateway/"));
    ...
    // Reset agents.defaults.model.primary if it uses ES Gateway
    if (settings.agents?.defaults?.model?.primary?.startsWith("ES Gateway/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "ES Gateway settings removed successfully",
    });
  } catch (error) {
