...
// Check if settings has ES Gateway customModels
const hasESGatewayConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id === "custom:ES Gateway-0");
};
...
// POST - Update ES Gateway customModels (merge with existing settings)
export async function POST(request) {
  try {
    ...
    // Remove existing ES Gateway config if any
    settings.customModels = settings.customModels.filter(m => m.id !== "custom:ES Gateway-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new ES Gateway config
    const customModel = {
      model: model,
      id: "custom:ES Gateway-0",
      index: 0,
...
// DELETE - Remove ES Gateway customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove ES Gateway customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => m.id !== "custom:ES Gateway-0");
