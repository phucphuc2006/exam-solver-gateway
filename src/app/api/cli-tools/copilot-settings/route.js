...
const hasESGatewayConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "ES Gateway");
};

const getESGatewayEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "ES Gateway") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getESGatewayEntry(config);
...
// POST - Apply ES Gateway config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing ES Gateway entry or append
    const idx = config.findIndex((e) => e.name === "ES Gateway");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove ES Gateway entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "ES Gateway");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "ES Gateway removed from Copilot config",
    });
  } catch (error) {
