...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  useEffect(() => {
    if (status?.config?.model?.startsWith("ES Gateway/")) {
      setSelectedModel(status.config.model.replace("ES Gateway/", ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    const url = status.config?.provider?.["ES Gateway"]?.options?.baseURL || "";
    const isLocal = url.includes("localhost") || url.includes("127.0.0.1");
    return status.hasESGateway && (isLocal || url.includes(baseUrl)) ? "configured" : status.hasESGateway ? "other" : "not_configured";
  };
  ...
  const handleApply = async () => {
    ...
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_ES Gateway" : selectedApiKey);
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_ES Gateway" : "<API_KEY_FROM_DASHBOARD>");
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `ES Gateway/${selectedModel || "provider/model-id"}`,
      }, null, 2),
    }];
  };
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current base URL */}
                {status?.config?.provider?.["ES Gateway"]?.options?.baseURL && (
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasESGateway} loading={restoring}>
