...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_ES Gateway for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# ES Gateway Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "ES Gateway"

[model_providers.es-gateway]
name = "ES Gateway"
          ...
          {!checkingCodex && codexStatus?.installed && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!codexStatus.hasESGateway} loading={restoring}>
